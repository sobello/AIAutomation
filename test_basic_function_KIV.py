...
#     assert "paris" in response_text, f"Expected 'paris' show in response', got '{response_text}'"
...
#     response = llm_client.generate_result_with_param(
#         prompt="What is the capital of Canada?",
#         max_tokens=10,
#         temperature=0.5,
#         assert_func=validate
#     )
#     print(f"\nModel response for Canada capital: '{response['text']}'")
...
def test_capital_of_canada(llm_client):
    ...
    def validate(response):
        ...
        assert "kenya" in response_text, (
            f"Expected 'kenya' in response, got '{response_text}'"
        )
        print(f"\n Model response for Canada capital: '{response_text}'")
